...
for idx, row in df.iterrows():
    for event in row['events']:
        path = f"../new/VideoLLaMA2/datasets/{event['label']}/{row['video']}_{str(event['frame'])}.mp4"
        if os.path.exists(path):
            labels.append((path, event['label']))

vds = VideoDataset(labels)
train_dataloader = DataLoader(vds, batch_size=4, shuffle=True)
...
# model.load_state_dict(torch.load('./final_model_stc', weights_only=True))
...
for epoch in range(epochs):
    ...
    for batch_num, input_data in enumerate(train_dataloader):
        ...
        optimizer.step()
        if batch_num % 40 == 0:
            print('\tEpoch %d | Batch %d | Loss %6.2f' % (epoch, batch_num, loss.item()), flush=True)
    print('Epoch %d | Loss %6.2f' % (epoch, sum(losses)/len(losses)))

torch.save(model.state_dict(), './final_model_stc2')
